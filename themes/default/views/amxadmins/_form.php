...
/**
 * @var Bans $model
 * Форма добавления/редактирования AmxModX админ
 */
...
$form=$this->beginWidget('bootstrap.widgets.TbActiveForm',array(
    ...
    if(!$model->isNewRecord && $model->flags != 'a' && $model->password) {
        $htmlOptions = array(
            'append' => '<label>'.CHtml::checkBox('removePwd') . ' Премахнете паролата</label>',
            'style' => 'width: 167px',
	...
	echo $form->textFieldRow(
		...
		array(
			'style' => 'width: 233px',
			'append' => '<span id="flagsselector" style="cursor: pointer">Избери</span>'
		)
	);

	echo $form->textFieldRow($model,'icq',array('class' => 'span6',));
	echo $form->dropDownListRow($model,'ashow', array('Не', 'Да'),array('class' => 'span6',));
	...
	<label for="Amxadmins_change">Промяна на админ крайния срок</label>
	<div class="row-fluid">
		<div class="span2">

			<label class="radio"><input id="Amxadmins_addtake_0" value="0" type="radio" name="Amxadmins[addtake]" checked /> Добавете</label>
			...
			<label class="radio"><input id="Amxadmins_addtake_2" value="2" type="radio" name="Amxadmins[addtake]"<?php echo $model->expired == 0 ? ' checked="checked"' : ''?> /> Завинаги</label>
		</div>
	...
	if($model->isNewRecord):?>
		...
		<button class="btn btn-info" type="button" onclick="$('#webrights').slideToggle('slow');">Добавете WEB администратор</button>
	...
	<div class="form-actions">
		<?php $this->widget('bootstrap.widgets.TbButton', array(
			'buttonType'=>'submit',
			'type'=>'primary',
			'label'=>$model->isNewRecord ? 'Добавете' : 'Запазване',
		)); ?>
	</div>
