 ...
 * @property string $name Име комментатора
...
class Comments extends CActiveRecord
{
	...
	public function afterSave() {
		if(!$this->isNewRecord)
			Syslog::add(Logs::LOG_EDITED, 'Изменен коментар к бану № <strong>' . $this->bid . '</strong>, написанный пользователем <strong>' . $this->name . '</strong>');
		return parent::afterSave();
	}

	public function afterDelete() {
		Syslog::add(Logs::LOG_DELETED, 'Удален коментар к бану <strong>' . $this->bid . '</strong>, написанный пользователем <strong>' . $this->name . '</strong>');
