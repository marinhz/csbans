...
class AdminController   extends Controller
{
	...
	public function actionReasons()
	{
		...
		if(isset($_POST['groupid']))
		{
			...
			$js .= "<table class=\"table table-bordered table-condensed\"><thead><tr><th colspan=3 style=\"vertical-align: middle\">Име группы ".CHtml::textField('groupname', $group->setname)."</th></tr><tr><th>Причина</th><th>Период на бан</th><th>Действия</th></thead><tbody>";
			...
			$js .= CHtml::button('Запазване', array('class' => 'btn btn-primary save','onclick' => '$.post(\'\', $(\'#form'.$gid.'\').serialize(), function(data){eval(data);});'));
			$js .= "</td></tr>";
			$js .= "</tbody></table>";
			$js .= CHtml::endForm();

			Yii::app()->end("$('#loading').hide();$('.modal-header').html('<h2>Причини за бан для группы \"{$group->setname}\"</h2>');$('.modal-body').html('{$js}');$('#reasons-modal').modal('show');");
		}
		...
		if(isset($_POST['active']))
		{
			...
			if($set === NULL)
				Yii::app()->end("alert('Грешка! Группа причин под ID ".intval($_POST['gid'])." не найдена');");
	...
	public function actionActions()
	{
		...
		switch ($_POST['action'])
		{
			...
			case 'optimizedb':
				...
				$alert = $optimize ? "База оптимизирована" : "Грешка оптимизации";
	 ...
	 * @param string $dir Име папки, в которой нужно удалить файлы и папки рекурсивно
	 */
