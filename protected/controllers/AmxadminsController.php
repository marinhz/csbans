...
class AmxadminsController extends Controller
{
	...
	public function actionAdminsonservers()
	{
		...
		if(Yii::app()->request->isAjaxRequest && isset($_POST['sid']) && isset($_POST['aid'])) {
			...
			if(isset($_POST['active'])) {
				...
				if(!empty($_POST['customflags']) && !preg_match('#^([a-z]+)$#', $_POST['customflags']))
					Yii::app()->end("alert('Грешка!В флаги нужно прописывать только буквы латинского алфавита');");
		...
		elseif(Yii::app()->request->isAjaxRequest && isset($_POST['sid']) && is_numeric($_POST['sid']))
		{
			...
			// Шапка таблицы с админи
			$js = "<table class=\"table table-bordered table-stripped\"><thead><tr class=\"info\"><th colspan=5>{$server->hostname}</th></tr></tr><tr><th>Ник</th><th>Steam/IP/Ник</th><th>Доп. флаги</th><th>Установленное время бана</th><th>Вкл</th><th>Запазване</th></thead><tbody>";
	...
	public function actionView($id=NULL)
	{
		...
		// Если стимайди админ проходит проверку на валидность, получаем инфу об админе с вальве
		...
		// Если есть сервера у админ, выводим сервера
		if($model->servers)
		{
